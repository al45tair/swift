...
//
// This must work inside a non-privileged Docker container, which places some
// significant restrictions on what it can and cannot do.  In particular, it
// will not be able to use process_vm_readv() as we will not have the PTRACE
// capability.
//
//===----------------------------------------------------------------------===//
...
void suspend_other_threads(swift::ThreadInfo *self);
...
_swift_installCrashHandler()
{
  ...
  // Do a sigsetjmp/siglongjmp; this seems pointless, but if we get to
  // the memory reader thread and the PLT lookup hasn't been done, when
  // we get a fault while doing a memcpy(), the dynamic linker stub will
  // try to do a PLT lookup and fail horribly, at which point the process
  // gets killed.
  if (!sigsetjmp(jb, 1)) {
    siglongjmp(jb, 1);
  }
...
{
  int old_err = errno;
  swift::ThreadInfo self = { 0, (int64_t)gettid(), (uint64_t)uctx };
  ...
  // The in-process memserver will have set signal handlers; reset them
  signal(SIGSEGV, SIG_DFL);
  signal(SIGBUS, SIG_DFL);

  // If we get here, we're trying to fall out and actually crash
  resume_other_threads();

  // Finally, restore errno
  errno = old_err;
}

void
reset_threads(swift::ThreadInfo *first) {
  __atomic_store_n(&crashInfo.thread_list, (uint64_t)first, __ATOMIC_RELEASE);
}

void
add_thread(swift::ThreadInfo *thread) {
...
seen_thread(pid_t tid) {
  uint64_t next = __atomic_load_n(&crashInfo.thread_list, __ATOMIC_ACQUIRE);
  while (next) {
    swift::ThreadInfo *pthread = (swift::ThreadInfo *)next;
    if (pthread->tid == tid)
      return true;
    next = pthread->next;
  }
...
void
suspend_other_threads(swift::ThreadInfo *self)
{
...
struct MemserverRequest {
  __swift_uint64_t addr;
  __swift_uint64_t len;
};

struct MemserverResponse {
  __swift_uint64_t addr;
  __swift_int64_t  len;
...
run_backtracer(int memserver)
{
  ...
  // Actually execute it
  return _swift_spawnBacktracer(backtracer_argv, memserver);
}
...
// Fetch memory from the remote memory server
extern "C" SWIFT_CC(swift)
int _swift_fetchRemoteMemory(uint64_t from,
                             void *to,
                             uint64_t byteCount) {
  const int memserver_fd = 4;
  MemserverRequest request = { from, byteCount };
  ssize_t ret = write(memserver_fd, &request, sizeof(request));
  ...
  uint64_t todo = byteCount;
  while (todo) {
    MemserverResponse response;

    ret = read(memserver_fd, &response, sizeof(response));
    ...
    if (uint64_t(response.len) > todo) {
      return EIO;
    }

    ret = read(memserver_fd, ptr, response.len);
    if (ret < 0) {
      return errno;
    }
    if (ret != response.len) {
